...
impl Tui {
    ...
    fn move_cursor(&mut self, x: usize, y: usize) -> Result<(), std::io::Error> {
        ...
        write!(
            self.stdout,
            "{}",
            cursor::Goto(x + LEFT_OFFSET + 1, y + TOP_OFFSET + 1)
        )?;
        Ok(())
    }
    ...
    fn render_text_with_max_width(&mut self, line: &str, max_width: u16) -> Result<(), Error> {
        ...
        for c in line.graphemes(true) {
            let width: u16 = c
                .width()
                .try_into()
                .map_err(|_| Error::IncompatibleCharacter)?;
            current_width += width;
    ...
    fn render_list(
        ...
        let lines_to_render = if ellipsis_at_end {
            max_lines - 1
        } else {
            num_items
        };
        ...
        else if *scroll > num_items - max_lines {
            *scroll = num_items - max_lines;
        }
        ...
        else if *scroll + lines_to_render <= selected {
            *scroll = selected - lines_to_render + 1;
        }

        // Recalculate formatting as scroll may have changed
        let ellipsis_at_end = requires_scroll && *scroll + max_lines < num_items;
        let num_lines_to_render = if ellipsis_at_end {
            max_lines - 1
        } else {
            num_items
        };

        let render_lines = items
            .iter()
    ...
    pub(super) fn new_frame(&mut self) -> Result<(), TuiError> {
        ...
        write!(
            self.stdout,
            "{}",
            BORDER_PATTERN_VERTICAL
                .chars()
                .cycle()
                .take(h as usize)
                .map(|s| format!("{s}{}{}", cursor::Down(1), cursor::Left(1)))
                .collect::<String>()
        )?;
    ...
    pub(super) fn render_text_centred(&mut self, text: &str, line: u16) -> Result<(), Error> {
        ...
        write!(
            self.stdout,
            "{}",
            cursor::Goto(left_offset + LEFT_OFFSET + 1, line)
        )?;
    ...
    /// ### Panics:
    /// * If the terminal is too small, based on if [`get_size_checked`] fails
    pub(super) fn render_graphemes_from_str<'a: 'b, 'b>(
        &mut self,
        text: &'a str,
        graphemes: usize,
        layout: &'b mut TextLayout<'a>,
    ) -> Result<(), Error> {
        ...
        for (screen_line, (layout_line, line)) in layout
            .lines
            .iter()
            .enumerate()
            .skip(lines_to_skip)
            ...
            .enumerate()
        {
            ...
            // If this is the last line for this frame, print only the required number of graphemes
                ...
                let (end_index, _) = line
                    ...
                    .nth(graphemes - graphemes_so_far)
                    .unwrap();
                self.move_cursor(0, screen_line)?;
